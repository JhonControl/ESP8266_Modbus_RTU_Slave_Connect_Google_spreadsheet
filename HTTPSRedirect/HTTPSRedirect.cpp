...
//#define DEBUG   //If you comment this line, the DPRINT & DPRINTLN lines are defined as blank.
  ...
  #define DPRINT(...)    Serial1.print(__VA_ARGS__)     //DPRINT is a macro, debug print
  #define DPRINTLN(...)  Serial1.println(__VA_ARGS__)   //DPRINTLN is a macro, debug print with new line
#else
...
bool HTTPSRedirect::printRedir(const char* url, const char* host, const char* redirHost){
  ...
  if (!connected()){
    Serial1.println("Error! Not connected to host.");
    return false;
  }
  ...
  if (!connect(redirHost, httpsPort)) {
    Serial1.println("Connection to re-directed host failed!");
    return 0;
  }

  if (fpCheck){
    if (verify(redirFingerprint, redirHost)) {
      Serial1.println("Re-directed host certificate match.");
    } else {
      Serial1.println("Re-directed host certificate mis-match");
    }
  }
